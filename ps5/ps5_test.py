...
class TestPS5(unittest.TestCase):
	...
	def test_generate_models(self):
		...
		array_type_msg = "each model returned by generate_models should be of type numpy.array"
		coefficient_mismatch = "coefficients of returned model are not as expected"

		# simple y = x case. 
		x = numpy.array(range(50))
		y = numpy.array(range(50))
		degrees = [1]
		...
		self.assertIsInstance(models[0], numpy.ndarray, array_type_msg)
		self.assertListEqual(list(models[0]), list(numpy.polyfit(x, y, 1)), coefficient_mismatch)

		# two models for y = 2x case 
		y = numpy.array(range(0,100,2))
		degrees = [1, 2]
		models = ps5.generate_models(x, y, degrees)
		self.assertEqual(len(models), len(degrees), degs_msg)
		self.assertIsInstance(models, list, list_type_msg)
		for m in models:
			self.assertIsInstance(m, numpy.ndarray, array_type_msg)
		for i in range(2):
			self.assertListEqual(list(models[i]), list(numpy.polyfit(x,y, degrees[i])), coefficient_mismatch)
	...
	def test_gen_cities_avg(self):
		...
		test_years = numpy.array(ps5.TESTING_INTERVAL)
	...
	def test_moving_avg(self):
		y = [1, 2, 3, 4, 5, 6, 7]
		window_length = 3
		correct = numpy.array([1, 1.5, 2, 3, 4, 5, 6])
	...
	def test_rmse(self):
		...
		result = ps5.rmse(numpy.array(y), numpy.array(estimate))
	...
	def test_gen_std_devs(self):
		climate = ps5.Climate('data.csv')
		years = numpy.array(ps5.TRAINING_INTERVAL)

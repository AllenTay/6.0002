    ...
    with open(map_filename, 'r') as file:
        for line in file:
            ...
            try:
                ...
                ValueError()
            g.add_edge(WeightedEdge(src, dest, int(total), int(outdoor)))
    return g
...
def get_best_path(digraph, start, end, path, max_dist_outdoors, best_dist,
    ...
    if start == end:
        ...
        for node in digraph.get_edges_for_node(Node(start)):
            ...
            if str(dest) not in curr_path:
                new_total = total_dist + node.get_total_distance()
                new_outdoor = outdoor_dist + node.get_outdoor_distance()
                outdoor_left = max_dist_outdoors - new_outdoor
                if outdoor_left >= 0 and total_dist < best_dist:
                    newPath = get_best_path(digraph, str(dest), end, [curr_path, new_total, new_outdoor], max_dist_outdoors, best_dist, best_path)
                    if newPath != None:
